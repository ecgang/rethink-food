...
### 17. Kitchen & field ops as field-PWA surfaces, reusing the existing primitives
**Why.** The posting's "Kitchen and Field Operations Tools" bullet lists eight capabilities (production planning, inventory, meal counts, food safety, QA, delivery, incident tracking, partner communication). Rather than a new app, these extend the existing `/field` PWA so they inherit its install/offline shell, role gate, and server-action patterns. This round closes the lifecycle (a **`markProduced`** action — `PLANNED→PRODUCED` previously had no in-app control, so production was silently assumed) and adds the two highest-signal surfaces: **food-safety/QA checklists** (`SafetyCheck` + pure `lib/safety.ts` engine, FDA cold-holding rule) and **incident tracking** (`Incident` + pure `lib/incidents.ts`). Both integrate rather than isolate: an open HIGH/CRITICAL incident and a recent failed safety check become live `detectExceptions` rules, so they appear on the dashboard "act on today" feed and the AI briefing automatically; an incident can spawn an `INCIDENT_NOTICE` through the existing draft-and-approve comms loop (ADR 16), covering "partner communication" without a new send path. All new actions reuse the existing **`operate:field`** capability — no RBAC sprawl.
**Tradeoff.** Inventory and deep production planning (shifts, recipes, batch yields) are deferred — "production planning / meal counts" is covered only by the produce queue + per-stage counts this round. The checklist item sets live in code, not a DB-editable template. Reusing `operate:field` means incident *resolution* isn't separately gated from *reporting*.
**Revisit if.** Ops wants editable checklist templates (move item sets to a table), inventory/lot tracking, or a distinct `manage:incident` capability for resolution vs reporting.
